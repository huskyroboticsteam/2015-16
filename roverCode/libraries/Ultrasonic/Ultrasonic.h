// ---------------------------------------------------------------------------
// Ultrasonic Library - v1.5 - 08/15/2012
//
// AUTHOR/LICENSE:
...
#define MAX_SONAR_DISTANCE 500 // Maximum sensor distance can be as high as 500cm, no reason to wait for ping longer than sound takes to travel this distance and back.
...
#define NO_ECHO 0               // Value returned if there's no ping echo within the specified MAX_SONAR_DISTANCE or max_cm_distance.
#define MAX_SONAR_DELAY 18000  // Maximum uS it takes for sensor to start the ping (SRF06 is the highest measured, just under 18ms).
...
#define UltrasonicConvert(echoTime, conversionFactor) (max((echoTime + conversionFactor / 2) / conversionFactor, (echoTime ? 1 : 0)))


class Ultrasonic {
	public:
		Ultrasonic(uint8_t trigger_pin, uint8_t echo_pin, int max_cm_distance = MAX_SONAR_DISTANCE);
		unsigned int ping();
